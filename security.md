...
involves a certain degree of complexity, KUndan tries to hide this complexity
...
While KUndans open source nature is helpful in uncovering unintended
...
This means that unlike other products, KUndan does not require explicit trust
between the end user and the host.

## Encryption

Robust data encryption is the foundation of KUndans security architecture.
KUndan utilizes three basic encryption schemes.

### Simple Symmetric Encryption

This is the most basic encryption scheme used in KUndan. Simple encryption
...
transmitted securely. KUndan currently uses the AES cipher in GCM mode, but
...
The **Account** object represents an individual KUndan user and is central to
KUndans encryption and authentication mechanisms. Each **Account** holds the
...
want to protect from prying eyes. In KUndan, this data is stored within
so-called **Vaults**.
...
Each KUndan user owns a private vault to which only they have access. In
...
and data transfer is generally mediated by a central server, KUndans
...
Instead of exchanging keys between all organization members directly, KUndan
...
Even though all sensitive information in **KUndan** is end-to-end encrypted and
theoretically secure even in case of an insecure connection or even a
compromised server, **KUndan** still uses a robust authentication scheme to
...
Whenever a user creates a KUndan account, the following steps take place:
...
-   Communication between the KUndan client and server is always secured through
    [Transport Layer Security](https://en.wikipedia.org/wiki/Transport_Layer_Security).
-   No sensitive information is ever transmitted in plain text.
-   Transmitted data is protected from tampering through KUndan's strong
    [Authentication Mechanism](#authentication-and-data-transfer).
-   KUndan's [Key Exchange Mechanism](#trustless-server-mediated-key-exchange)
    is designed to be secure even over an untrusted connection.

### Phishing

With the addition of
[Organizations And Shared Vaults](#organizations-and-shared-vaults) in KUndan 3,
phishing has become a potential attack vector as well. Attackers may try to lure
KUndan users into sharing sensitive information by inviting them to misleadingly
...
KUndan uses a combination of various
...
KUndan user themself. Unfortunately, since this means that in the majority of
